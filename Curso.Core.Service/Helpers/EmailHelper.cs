...
namespace curso.Core.Service.Helpers
{
    ...
    {
        public static void SendMail(string recipient, string subject, string message, bool isBodyHtml = false)
        {
            SendMail(new List<string>() { recipient }, subject, message, isBodyHtml);
        }

        public static void SendMail(List<string> recipitentsMail, string subject, string message, bool isBodyHtml = false)
        {
            try
            {
                var configuration = ConfigureSettings.GetEmailConfig();
                MailMessage mail = new MailMessage();
                mail.From = new MailAddress(configuration.Sender);
                recipitentsMail.ForEach(email =>
                {
                    mail.To.Add(email);
                });
                SmtpClient client = new SmtpClient();
                client.UseDefaultCredentials = false;
                client.Credentials = new NetworkCredential(configuration.Password, configuration.Login);
        ...
        public static async Task<dynamic> SendMail(string body, Tuple<string, string> sender, string template, string subject, Tuple<string, string> email)
        {
            var emails = new List<EmailAddress>();
            if (email != null)
            {
                emails.Add(new EmailAddress() { Email = email.Item1, Name = email.Item2 });
            }
            return SendMail(body, sender, template, subject, emails);
        }

        public static async Task<dynamic> SendMail(string body, Tuple<string, string> sender, string template, string subject, List<Tuple<string, string>> emails)
        {
            ...
            if (emails != null && emails.Count > 0)
            {
                ...
                {
                    emailList.Add(new EmailAddress() { Email = email.Item1, Name = email.Item2 });
                });
            }
            return SendMail(body, sender, template, subject, emailList);
        }

        public static async Task<dynamic> SendMail(string body, Tuple<string, string> sender, string template, string subject, List<EmailAddress> emails)
        {
            sender = sender ?? new Tuple<string, string>("contato@behavy.com", "Behavy");
            template = string.IsNullOrWhiteSpace(template) ? "a006c5d5-3306-4374-839d-4826a7c426b7" : template;

            var configuration = ConfigureSettings.GetEmailConfig();
            if (body.IsNullOrEmpty()) body = "<p></p>";
            ...
            while (currentPage <= pages)
            {
                ...
                if (items.Count > 0)
                {
                    ...
                    List<Dictionary<string, string>> substituitions = new List<Dictionary<string, string>>();
                    foreach (var item in items)
                    {
                        substituitions.Add(new Dictionary<string, string> { { "-Name-", item.Name } });
                        listSubjects.Add(subject);
                    }

                    var msg = MailHelper.CreateMultipleEmailsToMultipleRecipients(from, items, listSubjects, plainTextContent, htmlContent, substituitions);
